...
The `cbcd` 0.80.0 release introduces several enhancements that improve the reliability, scalability and security of the XRP Ledger.
...
The `cbcd` 0.60.3 release helps to increase the stability of the network under heavy load.
...
cbcd 0.29.0 has been released. The commit can be found on GitHub at: <https://github.com/cbc/cbcd/commits/0.29.0>
...
This release incorporates a significant number of improvements and important bugfixes. Please refer to the [Git commit history](https://github.com/cbc/cbcd/commits/develop) for more detailed information.
...
The minimum supported version of libBOOST is v1.55. You **must** upgrade to this release or later to successfully compile this release. Please follow [these instructions](https://wiki.cbc.com/Ubuntu_build_instructions#Install_Boost) if you have not upgraded already.
...
cbcd version 0.21.0 has been released. This release is currently the tip of the **develop/** branch and can be found on GitHub at [1](https://github.com/cbc/cbcd/tree/develop). The tag is **0.21.0-rc2** and can be found on GitHub at [2](https://github.com/cbc/cbcd/tree/0.21.0-rc2).
...
As with the previous release, the minimum supported version of GCC used to compile cbcd is v4.8.
...
If you are upgrading from version 0.12 or earlier of cbcd, these next sections apply to you because the format of the *cbcd.cfg* file changed around that time. If you have upgraded since that time and you have applied the configuration file fixes, you can safely ignore them.

**Validators**

cbc Labs is now running five validators. You can use this template for your *validators.txt* file (or place this in your config file):
...
You should also raise your quorum to at least three by putting the following in your *cbcd.cfg* file:
...
A list of cbc Labs server IP addresses can be found by resolving **r.cbc.com**. You can also add this to your *cbcd.cfg* file to ensure you always have several peer connections to cbc Labs servers:
...
If you are switching over from an existing back end, you have two options. You can remove your old database and let cbcd recreate it as it re-syncs, or you can import your old database into the new one.

To remove your old database, make sure the server is shut down (\`cbcd stop\`). Remove the *db/ledger.db* and *db/transaction.db* files. Remove all the files in your back end store directory (*db/hashnode* by default). Then change your configuration file to use the RocksDB back end and restart.

To import your old database, start by shutting the server down. Then modify the configuration file by renaming your *\[node\_db\]* stanza to *\[import\_db\]*. Create a new *\[node\_db\]* stanza and specify a RocksDB back end with a different directory. Start the server with the command **cbcd --import**. When the import finishes gracefully stop the server (\`cbcd stop\`). Please wait for cbcd to stop on its own because it can take several minutes for it to shut down after an import. Remove the old database, put the new database into place, remove the *\[import\_db\]* section, change the *\[node\_db\]* section to refer to the final location, and restart the server.
...
You need to configure the [NodeBackEnd](https://wiki.cbc.com/NodeBackEnd) that you want the server to use. See above for an example RocksDB configuration.
...
cbcd version 0.20.1 has been released. This release is currently the tip of the [develop](https://github.com/cbc/cbcd/tree/develop) branch and the tag is [0.20.1](https://github.com/cbc/cbcd/tree/0.20.1).
...
-   cbcd will crash randomly.
    -   Entries in the three parts of the order book are missing or do not match. In such a case, cbcd will crash.
-   Server loses sync randomly.
    -   This is due to cbcd restarting after it crashes. That the server restarted is not obvious and appears to be something else.
-   Server goes 'offline' randomly.
    -   This is due to cbcd restarting after it crashes. That the server restarted is not obvious and appears to be something else.
-   **complete\_ledgers** part of **server\_info** output says "None".
    -   This is due to cbcd restarting and reconstructing the ledger after it crashes.
    -   If the node back end is corrupted or has been moved without being renamed in cbcd.cfg, this can cause cbcd to crash and restart.

**Toolchain support**

Starting with this release, the minimum supported version of GCC used to compile cbcd is v4.8.
...
-   Reset liquidity before retrying cbcCalc.
...
-   Added Debian-style initscript (doc/cbcd.init).
-   Updated default config file (doc/cbcd-example.cfg) to reflect best practices.
...
cbcd version 0.19 has now been released. This release is currently the tip of the [release](https://github.com/cbc/cbcd/tree/release) branch and the tag is [0.19.0](https://github.com/cbc/cbcd/tree/0.19.0).
...
cbcd version 0.16 has now been released. This release is currently the tip of the [master](https://github.com/cbc/cbcd/tree/master) branch and the tag is [v0.16.0](https://github.com/cbc/cbcd/tree/v0.16.0).
...
-   cbcd no longer throws and exception on exiting
-   Better error reporting
-   cbc-lib tests have been ported to use the Mocha testing framework

**Validators**

cbc Labs is now running five validators. You can use this template for your `validators.txt` file:
...
You need to configure the [NodeBackEnd](https://wiki.cbc.com/NodeBackEnd) that you want the server to use. In most cases, that will mean adding this to your configuration file:
...
cbcd version 0.14 has now been released. This release is currently the tip of the [master](https://github.com/cbc/cbcd/tree/master) branch and the tag is [v0.12.0](https://github.com/cbc/cbcd/tree/v0.14.0).
...
cbcd version 0.12 has now been released. This release is currently the tip of the [master branch](https://github.com/cbc/cbcd/tree/master) and can be found on GitHub. The tag is [v0.12.0](https://github.com/cbc/cbcd/tree/v0.12.0).
...
This issue was caused by LevelDB periodically compacting its internal data structure. While compacting, cbcd's processing would stall causing the node to lose sync with the rest of the network. This issue was solved by switching from LevelDB to HyperLevelDB. cbcd operators will need to change their cbc.cfg file. See below for configuration details.
...
Occasionally, some cbcd servers would fail to fetch all ledgers. This left gaps in the local history and caused some API calls to report incomplete results. The ledger fetch code was rewritten to both prevent this and to repair any existing gaps.
...
**Update April 2014** - Due to a vulnerability in OpenSSL the validator keys above have been cycled out, the five validators by cbcLabs use the following keys now:
