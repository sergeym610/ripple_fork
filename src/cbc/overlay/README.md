...
The _cbc payment network_ consists of a collection of _peers_ running
**cbcd**. Each peer maintains multiple outgoing connections and optional
...
fully connected directed graph of nodes where vertices are instances of cbcd
...
establishment, defines the _cbc peer protocol_ (_protocol_ in this context).
...
* _User Defined_ (Unimplemented)
    ...
    The cbcd operator may specify additional, optional fields and values
    through the configuration. These headers will be transmitted in the
    corresponding request or response messages.

# cbc Clustering #

A cluster consists of more than one cbc server under common
...
Cluster members are configured in the `cbcd.cfg` file under
