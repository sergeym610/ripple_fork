...
We do not recommend Windows for cbcd production use at this time. Currently, the Ubuntu
...
build cbcd under Visual Studio, and run the unit tests you will need these
software components:
...
Any version of Visual Studio 2015 may be used to build cbcd.
...
* NOTE: To gain full featured access to the
  ...
  functionality used in the cbcd repository we suggest Git version 2.6.2 or later.

### Install Google Protocol Buffers Compiler

Building cbcd requires **protoc.exe** version 2.5.1 or later. At your option you
...
[protoc.exe](https://cbc.github.io/Downloads/protoc/2.5.1/protoc.exe)
([alternate link](https://github.com/cbc/Downloads/raw/gh-pages/protoc/2.5.1/protoc.exe))
precompiled Windows executable from the
[cbc Organization](https://github.com/cbc).
...
cbcd with Visual Studio, but can be used to build from the
command line and in scripts, and are required to properly update
the `cbcD.vcxproj` file.
...
2. 64-bit light - Don't use this. It is missing files needed to build cbcd. As of this writing, the link is called: "Win64 OpenSSL v1.0.2j Light"
...
* NOTE: Since cbcd links statically to OpenSSL, it does not matter
  where the OpenSSL .DLL files are placed, or what version they are.
  cbcd does not use or require any external .DLL files to run
...
The cbcd application is linked statically to the standard runtimes and external
...
### Clone the cbcd repository

If you are familiar with cloning github repositories, just follow your normal process
and clone `git@github.com:cbc/cbcd.git`. Otherwise follow this section for instructions.
...
step above. Navigate to the directory where you want to clone cbcd (git
...
```powershell
git clone git@github.com:cbc/cbcd.git
cd cbcd
git checkout master
```
...
Open the solution file located at **Builds/Visual Studio 2015/cbc.sln**
...
## Create a working directory for cbcd.cfg

The cbcd server uses the [cbcd.cfg](https://wiki.cbc.com/cbcd.cfg)
...
the cbcd server where that file is.

1. Create a directory to hold the configuration file. In this example, the
  cbc config directory was created in `C:\Users\joe\cbc\config`.
2. Copy the example config file located in `doc\cbcd-example.cfg` to the
  new directory and rename it "cbcd.cfg".
3. Read the cbcd.cfg file and edit as appropriate.

## Change the Visual Studio Projects Debugging Properties

1. If not already open, open the solution file located at **Builds/Visual Studio 2015/cbc.sln**
2. Select the correct solution platform in the solution platform dropdown (either *x64*
  or *Win32* depending on machine type).
3. Select the "Project->Properties" menu item to bring up cbcD's Properties Pages
...
### Tell cbcd where to find the configuration file.

The `--conf` command-line switch to tell cbcd where to find this file.
In the "Command Arguments" field in the properties dialog (that you opened
in the above section), add: `--conf="C:/Users/joe/cbc/config/cbcd.cfg"`
...
cbcd can run very slowly in the debugger when using the Windows Debug Heap.
...
After these steps are complete, cbcd should be ready to build. Simply
set cbcd as the startup project by right clicking on it in the
Visual Studio Solution Explorer, choose **Set as Startup Project**,
and then choose the **Build->Build Solution** menu item.

# Unit Tests (Recommended)

The cbcd unit tests are written in C++ and are part
of the cbcd executable.

From a Windows console, run the unit tests:

```
./build/msvc.debug/cbcd.exe --unittest
```
