...
We don't recommend macos for cbcd production use at this time. Currently, the
...
modification, build cbcd using clang, and run the system tests you will need
...
functionality used in the cbcd repository, we suggest Git version 1.8.3.2 or
later.
...
should satisfy the cmake minimum version requirement for cbcd.
...
Building cbcd on osx requires `protoc` version 2.5.x or 2.6.x (later versions
do not work with cbcd at this time).
...
[GitHub](https://github.com/cbc/cbcd/releases GitHub).
...
cbcd builds a set of unit tests into the server executable. To run these unit
tests after building, pass the `--unittest` option to the compiled `cbcd`
