## Heap profiling of cbcd with jemalloc
...
jemalloc is acquired separately from cbcd, and is not affiliated
with cbc Labs. If you compile and install jemalloc from the
...
profiling has been tested with cbcd on a Linux platform. It should
work on platforms on which both cbcd and jemalloc are available.

To link cbcd with jemalloc, the argument
...
Build cbcd with jemalloc library under /usr/local/lib and
header under /usr/local/include:

    $ scons profile-jemalloc=/usr/local

Build cbcd using clang with the jemalloc library under /opt/local/lib
